...
class Server extends \OAuth2\Server
{
    ...
    public function checkClientCredintials(\OAuth2\RequestInterface $request = null, \OAuth2\ResponseInterface $response = null) {
        if ($request === null) {
            $request = $this->module->getRequest();
        }
        $this->response = is_null($response) ? new Response() : $response;
        $http = new \OAuth2\ClientAssertionType\HttpBasic($this->storages['client_credentials']);
        $http->validateRequest($request, $this->response);
